<template>
  <div>
    <el-row type="flex" justify="end">
      <el-select
        v-model="currentYear"
        size="small"
        style="width: 100px"
        @change="Datehandle"
      >
        <el-option
          v-for="item in yearList"
          :key="item"
          :label="item"
          :value="item"
        />
      </el-select>
      <el-select
        v-model="currentMonth"
        size="small"
        style="width: 100px;margin-left: 10px"
        @change="Datehandle"
      >
        <el-option v-for="item in 12" :key="item" :value="item" :label="item" />
      </el-select>
    </el-row>
    <el-calendar v-model="currentDate">
      <template #dateCell="{ date, data }">
        <div class="date-content">
          <span class="text">{{ data.day | getDay }} </span>
          <span v-if="isWork(date)" class="rest">休</span>
        </div>
      </template>
    </el-calendar>
  </div>
</template>

<script>
export default {
  components: {},
  filters: {
    getDay(value) {
      const day = value.split('-')[2]
      return day.startsWith('0') ? day.substring(1) : day
    }
  },
  props: {
    startDate: {
      type: Date,
      default: () => new Date()
    }
  },
  data() {
    return {
      urrentMonth: null, // 当前月
      currentYear: null, // 当前年
      currentDate: null, // 当前日期
      yearList: []
    }
  },
  computed: {},
  watch: {},
  created() {
    this.currentMonth = this.startDate.getMonth() + 1 // 获取当前月
    this.currentYear = this.startDate.getFullYear() // 获取当前年
    this.yearList = Array.from(Array(11), (value, index) => {
      // 生成当前年的前五年和后五年
      return this.currentYear + index - 5
    })
    this.Datehandle()
  },
  mounted() {},
  methods: {
    Datehandle() {
      this.currentDate = new Date(`${this.currentYear}-${this.currentMonth}-1 `) // 已 1号为开始
    },
    isWork(date) {
      return date.getDay() === 6 || date.getDay() === 0
    }
  }
}
</script>

<style scoped lang="scss">
::v-deep .el-calendar-day {
  height: auto;
}
::v-deep .el-calendar-table__row td,
::v-deep .el-calendar-table tr td:first-child,
::v-deep .el-calendar-table__row td.prev {
  border: none;
}
.date-content {
  height: 40px;
  text-align: center;
  line-height: 40px;
  font-size: 14px;
}
.date-content .rest {
  color: #fff;
  border-radius: 50%;
  background: rgb(250, 124, 77);
  width: 20px;
  height: 20px;
  line-height: 20px;
  display: inline-block;
  font-size: 12px;
  margin-left: 10px;
}
.date-content .text {
  width: 20px;
  height: 20px;
  line-height: 20px;
  display: inline-block;
}
::v-deep .el-calendar-table td.is-selected .text {
  background: #409eff;
  color: #fff;
  border-radius: 50%;
}
::v-deep .el-calendar__header {
  display: none;
}
</style>
